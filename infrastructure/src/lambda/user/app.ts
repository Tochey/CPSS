...
app.get("/user/getAllStudents", async function (req: express.Request, res: express.Response) {
    return userModel.query('ROLE').eq('STUDENT').exec().then((data) => {
        res.status(200).send(data)
    })
})

app.get("/user/getAllFaculty", async function (req: express.Request, res: express.Response) {
    return userModel.query('ROLE').eq('FACULTY').exec().then((data) => {
        res.status(200).send(data)
    })
})

app.delete("/user/deleteStudent/:userId", async function (req: express.Request, res: express.Response) {
...
app.delete("/user/deleteFaculty/:userId", async function (req: express.Request, res: express.Response) {
...
app.post('/user/createTimeSlots', async (req, res) => {
    ...
    try {
        ...
        const time_slots = [];
        for (let time = st; time < et && time.getTime() + duration * 60 * 1000 <= et.getTime(); time.setMinutes(time.getMinutes() + duration)) {
        ...
        await Promise.all(time_slots.map(time_slot => timeSlotModel.create(time_slot)));
...
app.delete('/user/deleteTimeSlots', async (req, res) => {
    try {
        ...
        if (registrations.length === 0) {
            return res.status(200).json({ error: 'No registrations found but timeslot have been deleted' });
        }

        await registrationModel.batchDelete(registrations)

        return res.status(200).json({ message: 'All time slots and registrations have been deleted' });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ error: 'An error occurred while deleting time slots and registrations' });
    }
});
