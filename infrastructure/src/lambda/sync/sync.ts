...
type CustomSQSEvent = Omit<SQSEvent, "Records"> & {
  Records: Array<Omit<SQSEvent["Records"][0], "body"> & { body: CustomSQSBody }>;
};
  ...
  return await axios
    .get(`https://canvas.instructure.com/api/v1/courses/${courseId}/assignments`, {
      ...
      let capstoneSpecAssignment = e.data.find((e) =>
        e.name && (e.name.toLowerCase().includes("csc520 submission") || e.name.toLowerCase().includes("final Submission"))
      );
      ...
      if (!capstoneSpecAssignment) {
        ...
        while (nextLink && !capstoneSpecAssignment) {
          ...
          capstoneSpecAssignment = res.data.find(
            (e) => e.name && (e.name.toLowerCase().includes("csc520 submission") || e.name.toLowerCase().includes("final Submission"))
          )
...
const getCapstoneDocAttachmentIdIfPresent = async (courseId: number, assignmentId: number, userId: string, accessToken: string): Promise<number> => {
...
const Sync520Documents: SyncRespose = async (sid, accessToken) => {
  ...
  const attachmentId = await getCapstoneDocAttachmentIdIfPresent(courseId, capstoneId, sid, accessToken)
  ...
  const prefix = `${fullName[0].charAt(0) + fullName[1] + "_" + student_id + "/"
    }`
  const fileName = prefix + "520_FINAL_SUBMISSION.zip"
  const client = new S3Client({ region: "us-east-1" })
  const buffer = Buffer.from(fileContent)
  const command = new PutObjectCommand({
    Bucket: process.env.SUBMISSION_BUCKET_NAME || "primary-submission-bucket21312",
    Key: fileName,
 ...
 const res =  await userModel.update({ userId: sid }, { has_uploaded_520_capstone: true })
 console.log(res)
}
